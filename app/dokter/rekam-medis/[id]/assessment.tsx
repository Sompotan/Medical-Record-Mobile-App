...
export default function AssessmentPage() {
    ...
    const handleAddDiagnosis = () => {
        ...
        const exists = diagnosisList.some(
            (d) => d.jenisDiagnosis === opsiDiagnosis && (opsiDiagnosis === "Utama" || opsiDiagnosis === "Banding")
        );
    ...
    useDebouncedEffect(() => {
        if (!rekamMedisId || diagnosisList.length === 0 || isReadonly) return;

        const isValid = diagnosisList.every((d) => d.kodeKlinisId && d.jenisDiagnosis && d.deskripsi !== undefined);
        if (!isValid) return;
        ...
        >
            {!isReadonly && (
                <>
                    ...
                    <JenisDiagnosisSelect value={opsiDiagnosis} onChange={setOpsiDiagnosis} />
            ...
            {["Utama", "Banding", "Lain"].map((jenis) =>
                diagnosisList.some((d) => d.jenisDiagnosis === jenis) ? (
                    <View key={jenis} className="mt-6 bg-white p-4 rounded-xl">
                        <Text className="font-semibold text-xl mb-2">Diagnosa {jenis}</Text>
                        {diagnosisList
                            .filter((d) => d.jenisDiagnosis === jenis)
                            .map((item) => (
                                <DiagnosisItemCard
                                    key={item.kodeKlinisId}
                                    item={item}
